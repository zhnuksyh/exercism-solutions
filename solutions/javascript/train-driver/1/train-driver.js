...
export function fixListOfWagons(ids) {
  const [a, b, ...theRest] = ids;
  const fixedList = [...theRest, a, b];
...
export function correctListOfWagons(ids, missingWagons) {
  const [a, ...theRest] = ids;
  const correctedWagon = [a, ...missingWagons, ...theRest];

  return correctedWagon;
...
export function separateTimeOfArrival(information) {
  const {timeOfArrival, ...theRest} = information;
  return [timeOfArrival, theRest];
}
